...
import { getPhaseDuration, TIMER_PHASES } from '../constants/timer';
  ...
  return create<TimerStore>()(
    persist(
      (set, get): TimerStore => ({
        ...
        reset: () => {
          const state = get();
          const duration = getPhaseDuration(state.phase, defaultConfig);
          set({
        ...
        setPhase: (phase: TimerPhase) => {
          const duration = getPhaseDuration(phase, defaultConfig);
          set({
            phase,
            timeRemaining: duration,
            totalDuration: duration,
        ...
        completeSession: () => {
          ...
          if (state.phase === TIMER_PHASES.WORK) {
            ...
            if (newCompletedSessions % defaultConfig.sessionsUntilLongBreak === 0) {
